...
def push(String credentialsId, String dockerHubRepo, String imageTag) {
        withCredentials([usernamePassword(credentialsId: credentialsId, usernameVariable: 'DOCKER_HUB_USR', passwordVariable: 'DOCKER_HUB_PSW')]) {
            script {
                sh """
                    ...
                    /kaniko/executor --dockerfile="/Dockerfile" --context `pwd` --destination ${dockerHubRepo}:${imageTag}
                """
        }
    }
}
